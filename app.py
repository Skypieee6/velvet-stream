...
def load_content(query="korean"):
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for video_list in futures:
            if video_list:
                for v in video_list:
                    # Restored original image pathing logic to fix "Not Loading" issue
                    all_videos.append({
                        "id": v['id'],
                        "title": v['title'],
...
<body>
    ...
    <header class="sticky top-0 bg-black/95 z-[100] border-b border-white/10 backdrop-blur-md">
        <div class="flex items-center justify-between p-4">
            <h1 class="font-black text-xl tracking-tighter text-white">VELVET</h1>
            <div class="relative w-1/2">
                <input type="text" id="search-input" onchange="searchCustom()" placeholder="Search..." class="w-full bg-zinc-900 rounded-full py-2 px-4 text-xs text-white outline-none">
            </div>
        </div>
        <div id="cat-bar" class="flex space-x-4 overflow-x-auto text-[11px] font-bold text-zinc-500 whitespace-nowrap uppercase px-4 pb-3">
    ...
    <div id="section-player" class="hidden fixed inset-0 bg-black z-[500] overflow-y-auto pb-20">
        ...
        <div class="p-5">
            ...
            <h2 class="text-pink-500 font-bold uppercase text-xs mt-8 mb-4 tracking-widest">Related Videos</h2>
    ...
    <script>
        ...
        function manualRefresh() {
            ...
            renderGrid(allData.slice(0, 20), 'grid-home');
